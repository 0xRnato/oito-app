<template>
  <v-container column text-xs-center>
    <v-layout row>
      <v-flex md3 sm12 v-for="(offer, index) in getOffersEmployeeByUser" :key="index">
        <v-card>
...
export default {
  ...
  computed: {
    ...mapGetters(["getOffersEmployeeByUser"])
  },
  methods: {
    ...mapActions(["actionOffersEmployeeByUser", "actionCreateOffer"]),
    submit() {
  ...
  created() {
    this.actionOffersEmployeeByUser();
  }
};
</script>
